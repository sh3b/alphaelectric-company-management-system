...
-- --------------------------------------------------
-- Date Created: 01/23/2017 22:42:53
-- Generated from EDMX file: D:\Cloud\Stuff\IIU\Semester 5\VP\Project\AlphaElectric\AlphaElectric_DataAccessLayer\DBModel.edmx
-- --------------------------------------------------
...
GO
IF OBJECT_ID(N'[dbo].[FK_ProductView_inherits_Product]', 'F') IS NOT NULL
    ALTER TABLE [dbo].[Products_ProductView] DROP CONSTRAINT [FK_ProductView_inherits_Product];
GO
...
GO
IF OBJECT_ID(N'[dbo].[Products_ProductView]', 'U') IS NOT NULL
    DROP TABLE [dbo].[Products_ProductView];
GO
...
-- Creating table 'Products_ProductV'
CREATE TABLE [dbo].[Products_ProductV] (
    [ID] int  NOT NULL
);
GO
...
-- Creating primary key on [ID] in table 'Products_ProductV'
ALTER TABLE [dbo].[Products_ProductV]
ADD CONSTRAINT [PK_Products_ProductV]
    PRIMARY KEY CLUSTERED ([ID] ASC);
GO
...
-- Creating foreign key on [CustomerOrder_ID] in table 'Projects'
ALTER TABLE [dbo].[Projects]
...
-- Creating foreign key on [ID] in table 'Products_ProductV'
ALTER TABLE [dbo].[Products_ProductV]
ADD CONSTRAINT [FK_ProductV_inherits_Product]
    FOREIGN KEY ([ID])
    REFERENCES [dbo].[Products]
        ([ID])
    ON DELETE CASCADE ON UPDATE NO ACTION;
